...
export class MemoriaTecnicaAutoconsumoComponent {
  ...
  async generarPDF() {
    ...
    try {
      ...
      const setSelect = (name: string, value: string): boolean => {
        const nombreReal = resolverNombreCampoPdf(name);
        if (!nombreReal) {
          console.warn(`[MTD] Campo PDF no encontrado: ${name}`);
          return false;
        ...
        let valorSeleccion = value;
        try {
          const field: any = form.getField(nombreReal);
          const opciones = field?.getOptions?.();
          if (Array.isArray(opciones)) {
            const codigo = extraerCodigoOpcion(value);
            const encontradaPorCodigo = codigo
              ? opciones.find(
                  (op: string) =>
                    normalizarTextoOpcion(op).startsWith(`${codigo} `) ||
                    normalizarTextoOpcion(op).startsWith(`${codigo}(`),
                )
              : undefined;
            const encontradaPorTexto = opciones.find((op: string) => {
              const normalizada = normalizarTextoOpcion(op);
              const base = normalizarTextoOpcion(value);
              return (
                normalizada === base ||
                normalizada.includes(base) ||
                base.includes(normalizada)
              );
            });
            const encontrada = encontradaPorCodigo || encontradaPorTexto;
            if (encontrada) {
              valorSeleccion = encontrada as string;
            } else {
              console.warn(
                `[MTD] Opcion no encontrada para ${nombreReal}: ${value}`,
              );
              return false;
            }
          }
          if (typeof field?.select === 'function') {
            field.select(valorSeleccion);
            return true;
          }
        } catch {}

        try {
          const f = form.getDropdown(nombreReal);
          f.select(valorSeleccion);
          return true;
        } catch {}

        try {
          const f = form.getOptionList(nombreReal);
          f.select(valorSeleccion);
          return true;
        } catch {}
        ...
        try {
          const f = form.getTextField(nombreReal);
          f.setText(valorSeleccion?.toString().toUpperCase() || '');
          return true;
        } catch (e) {
          console.warn(`[MTD] Campo PDF no encontrado: ${name}`);
          return false;
